...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        after_pack_padded = torch.nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(after_pack_padded)

        after_pad_padded, blank =  torch.nn.utils.rnn.pad_packed_sequence(enc_hiddens)

        enc_hiddens = after_pad_padded.permute(1,0,2)

        h_fowd = last_hidden[0]
        h_bowd = last_hidden[1]
          
        h_enc_concat = torch.cat([h_fowd, h_bowd], dim=1)
        init_decoder_hidden = self.h_projection(h_enc_concat)
          
        c_fowd, c_bowd = last_cell[0], last_cell[1]
        c_enc_concat = torch.cat([c_fowd, c_bowd], dim=1)
        init_decoder_cell = self.c_projection(c_enc_concat)
